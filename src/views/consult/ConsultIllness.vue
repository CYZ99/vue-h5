...
onMounted(() => {
  // 每次进入页面时,判断store是否存储有病情描述信息
  // 如果有则给用户提示,是否需要恢复之前填写的信息
...
<template>
  <!-- 3. 极速问诊-病情描述页面 -->
  <!-- 3.1 布局方面复用,导航组件,和选择卡片组件 -->
  <!-- 3.2 文本框描述病情,卡片选择时间和是否就诊过,卡片选择的选项和值对应上 -->
  <!-- 3.3 存放到 form 表单中,在将数据存放到 store 中 -->
  <!-- 3.4 上传图片功能使用  -->
  <!-- 3.5 按钮禁用功能,根据表单的必填项来判断 -->
  <!-- 3.6 页面回显功能,根据 store 中的是否存在数据来回显 -->
  <div class="consult-illness-page">
    ...
    <div class="illness-form">
      ...
      <van-button :class="{ disabled }" block round type="primary" @click="next">下一步</van-button>
    </div>
  </div>
</template>
